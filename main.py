...
@app.get("/")
def read_root():
    return {"status": "online", "service": "AgentPay MVP API"}

@app.post("/api/pay-usdc")
def create_usdc_payment(data: PaymentRequest):
    try:
        # Burada Solana SPL-Token transfer talimatları ve cüzdan doğrulama işlemleri yer alır
        if data.amount <= 0:
            raise HTTPException(status_code=400, detail="Geçersiz tutar.")
            
        return {
            "success": True,
            "message": "USDC ödeme talebi oluşturuldu.",
            "token": "USDC",
